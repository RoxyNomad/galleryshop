...
const Street = () => {
  ...
  const { selectedOption, sortOptions, pictures, handleSortChange } = useSort(
    filteredArtworks.filter((pic) => pic.category_id === streetCategoryId)
  );
  ...
  const handleColorChange = (color: string) => {
    if (color === '') {
      ...
      setFilteredArtworks(artworks.filter((pic) => pic.category_id === streetCategoryId));
    } else {
      // Filtere nach der ausgewählten Farbe
      setFilteredArtworks(
        artworks.filter(
          (pic) => pic.category_id === streetCategoryId && pic.base_color.toLowerCase() === color.toLowerCase()
        )
      );
    }
  };
